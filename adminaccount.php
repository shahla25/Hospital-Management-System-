...
<div class="wrapper col4">
  ...
  <p><form method="get" action=""><strong>Date -</strong> <input type="date" name="date" value="<?php echo $_GET[date]; ?>" ><input type="submit" name="submit" value="Submit"></form></p>
    ...
    <h1>Number of Room Availability  : 
    <?php
	$sql = "SELECT * FROM room WHERE status='Active'";
	if(isset($_GET[date]))
	{
		$sql = $sql . " AND room   ='$_GET[date]'";
	}
	$qsql = mysqli_query($con,$sql);
	echo mysqli_num_rows($qsql);
	?>
...
<hr>
    ...
    <h1>Number of Room Records : 0
    <?php
	$sql = "SELECT * FROM room_records WHERE status='Active'";
	$qsql = mysqli_query($con,$sql);
	echo mysqli_num_rows($qsql);
	?>
    </h1>
    <h1>Number of Room Schedule : 0
 ...
 <h1>Number of Doctor Timings Records : 
    ...
    <h1>Number of Patient Invoices Records :
    <?php
